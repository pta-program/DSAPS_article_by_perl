...
```python
"""Floyd-Warshall 求全源最短路，再最小化每只动物的最大最短距离。"""
...
def main():
    ...
    pos = 2
    for _ in range(m):
        u, v, weight = data[pos:pos + 3]
        pos += 3
        u -= 1
        v -= 1
        distance[u][v] = distance[v][u] = min(distance[u][v], weight)
    ...
    candidates = [
        ...
        if infinity not in row
    ]
    print("0" if not candidates else f"{min(candidates)[1]} {min(candidates)[0]}")
