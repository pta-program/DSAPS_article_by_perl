...
```python
"""正正、负负配对收益为正；双端贪心配对两类，Python 整数自动支持大结果。"""
...
def main():
    ...
    if not values:
        return
    pos = 0
    coupon_count = values[pos]
    pos += 1
    coupons = values[pos:pos + coupon_count]
    pos += coupon_count
    product_count = values[pos]
    pos += 1
    products = values[pos:pos + product_count]
    coupons.sort()
    products.sort()
    answer = 0
    while coupons and products and coupons[-1] > 0 and products[-1] > 0:
        answer += coupons.pop() * products.pop()
    while coupons and products and coupons[0] < 0 and products[0] < 0:
        answer += coupons.pop(0) * products.pop(0)
    print(answer)
