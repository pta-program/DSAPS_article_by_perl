...
```perl
# 根据缩进深度记录父结点，查询亲属关系时沿 parent 链向上追溯。
use strict;
use warnings;
...
my @lines = split /\n/, $input;
exit unless @lines;
my ( $n, $query_count ) = split ' ', $lines[0];
my ( @names, @parents, %last_at_depth );
for my $i ( 0 .. $n - 1 ) {
    my $line = $lines[ $i + 1 ];
    my ( $indent_text, $name ) = $line =~ /^(\s*)(.*)$/;
    my $indent = length($indent_text);
    my $depth  = int( $indent / 2 );
    my $parent =
      exists $last_at_depth{ $depth - 1 } ? $last_at_depth{ $depth - 1 } : -1;
    push @names,   $name;
    push @parents, $parent;
    $last_at_depth{$depth} = $#names;
    delete $last_at_depth{$_} for grep { $_ > $depth } keys %last_at_depth;
}
my %index;
$index{ $names[$_] } = $_ for 0 .. $#names;
my @output;
for my $i ( 0 .. $query_count - 1 ) {
    my @words = split ' ', $lines[ $n + 1 + $i ];
    my ( $x, $y ) = ( $words[0], $words[-1] );
    my $relation = join( ' ', @words[ 1 .. $#words - 1 ] );
    my ( $a, $b ) = ( $index{$x}, $index{$y} );
    my $result;
    if    ( $relation eq 'is a child of' )    { $result = $parents[$a] == $b }
    elsif ( $relation eq 'is the parent of' ) { $result = $parents[$b] == $a }
    elsif ( $relation eq 'is a sibling of' ) {
        $result = $a != $b && $parents[$a] == $parents[$b];
    }
    else {
        my $start  = $relation eq 'is a descendant of' ? $a : $b;
        my $target = $start == $a                      ? $b : $a;
        $result = 0;
        my $current = $parents[$start];
        while ( $current != -1 ) {
            if ( $current == $target ) { $result = 1; last }
            $current = $parents[$current];
        }
    }
    push @output, $result ? 'True' : 'False';
}
print join( "\n", @output ), "\n";
```
