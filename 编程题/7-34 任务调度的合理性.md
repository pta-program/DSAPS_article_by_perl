...
```perl
# Kahn 拓扑排序反复删除入度为零的任务，无法处理完全部任务即有环。
use strict;
use warnings;
...
for my $task ( 1 .. $n ) {
    my $count = $values[ $pos++ ];
    for ( 1 .. $count ) {
        my $dependency = $values[ $pos++ ];
        push @{ $graph[$dependency] }, $task;
        ++$indegree[$task];
    }
}
...
while ( $head < @queue ) {
    my $task = $queue[ $head++ ];
    ++$processed;
    for my $dependent ( @{ $graph[$task] } ) {
        push @queue, $dependent if --$indegree[$dependent] == 0;
    }
}
print $processed == $n ? "1\n" : "0\n";
```
