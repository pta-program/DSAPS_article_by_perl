...
```python
"""最后 3 个字母用 32 进制移位得到初址，再使用双向平方探测。"""
...
def main():
    ...
    if not tokens:
        return
    count, size = map(int, tokens[:2])
    table = [None] * size
    result = []
    for key in tokens[2:2 + count]:
        home = hash_key(key) % size
        for index in probe_positions(home, size):
