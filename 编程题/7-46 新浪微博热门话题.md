...
输入说明：输入首先给出一个正整数 n（≤ 10^5），随后 n 行，每行给出一条英文微博，其长度不超过 140 个字符。任何包含在一对最近的“#”中的内容均被认为是一个话题，如果长度超过40个字符，则只保留前40个字符。输入保证“#”成对出现。
...
```perl
# 主题先截取 40 个字符并规范化；同一条微博内重复主题只计一次。
use strict;
use warnings;
...
for ( 1 .. $n ) {
    ...
    my $line = shift @lines;
    while ( $line =~ /#(.*?)#/g ) {
        my $topic = normalize( substr( $1, 0, 40 ) );
        $seen{$topic} = 1 if length $topic;
    }
    ++$count{$_} for keys %seen;
}
my $maximum = 0;
for my $topic ( keys %count ) {
    $maximum = $count{$topic} if $count{$topic} > $maximum;
}
my @winners = sort grep { $count{$_} == $maximum } keys %count;
my $display = $winners[0];
$display = ucfirst($display);
my @output = ( $display, $maximum );
push @output, 'And ' . ( @winners - 1 ) . ' more ...' if @winners > 1;
print join( "\n", @output ), "\n";
```
