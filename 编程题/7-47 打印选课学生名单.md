...
exit unless @tokens;
my ( $student_count, $course_count ) = @tokens[ 0, 1 ];
my @courses = map { [] } 0 .. $course_count;
my $pos     = 2;
for ( 1 .. $student_count ) {
    my $name  = $tokens[ $pos++ ];
    my $count = $tokens[ $pos++ ];
    for ( 1 .. $count ) { push @{ $courses[ $tokens[ $pos++ ] ] }, $name }
}
my @output;
for my $course ( 1 .. $course_count ) {
    my @students = sort @{ $courses[$course] };
    push @output, "$course " . scalar(@students), @students;
}
print join( "\n", @output ), "\n";
```
